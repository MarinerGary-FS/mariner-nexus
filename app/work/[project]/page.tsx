...
import { JacobIntelligenceRecord, JacobProductEvidence } from "@/components/work/JacobIntelligenceRecord";
...
export async function generateMetadata({ params }: ProjectPageProps): Promise<Metadata> {
  ...
  if (!project) return {};
  return createMetadata({ title: project.metadata.title, description: project.metadata.description, path: `/work/${project.slug}`, image: project.metadata.ogImage, imageAlt: `${project.clientName} — Transformation Record ${project.presentation.recordNumber} by Mariner Nexus.`, type: "article" });
}
...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  const isJacob = project.slug === "jacob-mariner" && Boolean(project.intelligenceRecord);
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "CreativeWork",
    "@id": `${absoluteUrl(`/work/${project.slug}`)}#transformation-record`,
    name: `${project.clientName} — Transformation Record ${project.presentation.recordNumber}`,
    headline: project.shortTransformation,
    abstract: project.metadata.description,
    url: absoluteUrl(`/work/${project.slug}`),
    image: absoluteUrl(project.metadata.ogImage),
    creator: { "@type": "Organization", name: siteName, url: absoluteUrl("/") },
  ...
  return <ProjectFrame identity={project.visualIdentity}>
    ...
    <ProjectIdentityBoundary experience={project.experienceProfile} identity={project.visualIdentity} project={project.slug} variant="record"><article>
      <header className={`mn-project-opening${isJacob ? " mn-jacob-opening" : ""}`} data-navigation-context="Work"><Container>{isJacob ? <><div className="mn-jacob-opening__identity"><div><p>{project.presentation.recordNumber} / Transformation Record</p><p>{project.presentation.taxonomy}</p></div><h1>{project.clientName}</h1></div><div className="mn-jacob-opening__statement"><h2>{project.shortTransformation}</h2><p>{project.presentation.heroSummary}</p><ExternalProjectLink href={project.liveUrl}>{project.presentation.heroCta}</ExternalProjectLink></div><MarinerThread label="Jacob Mariner’s identity leads into the operating athlete intelligence experience" nodes={3} tone="project" /><JacobProductEvidence media={project.media} /></> : <><div className="mn-project-opening-meta"><p>{project.clientName}</p><h1>{project.shortTransformation}</h1></div><MarinerThread label={`The ${project.clientName} identity leads into finished project proof`} nodes={2} tone="project" /><figure><Image alt={project.media.desktop.alt} fetchPriority="high" height={project.media.desktop.height} loading="eager" priority sizes="(max-width: 768px) calc(100vw - 40px), calc(100vw - 116px)" src={project.media.desktop.src} width={project.media.desktop.width} /><figcaption>The finished {project.clientName} digital experience.</figcaption></figure></>}</Container></header>

      {isJacob && project.intelligenceRecord ? <JacobIntelligenceRecord record={project.intelligenceRecord} /> : <>
