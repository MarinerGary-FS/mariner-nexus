...
export default function WorkPage() {
  return <div className="mn-work-index mn-environment-light">
    ...
    <section className="mn-work-projects"><Container>{publishedProjects.map((project) => <article className="mn-work-entry" key={project.slug}><Link aria-label={`View ${project.clientName} transformation record`} className="mn-work-media" href={`/work/${project.slug}`}><Image alt={project.media.card.alt} height={project.media.card.height} priority sizes="(max-width: 768px) calc(100vw - 40px), calc(100vw - 116px)" src={project.media.card.src} width={project.media.card.width} /></Link><div className="mn-work-entry-meta"><div><p>01 / Transformation Record</p><h2>{project.clientName}</h2></div><div><p>{project.shortTransformation}</p><Link href={`/work/${project.slug}`}>View Case Study</Link></div></div></article>)}</Container></section>
